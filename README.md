...
- macOS 26以降では新しい高精度オンデバイス認識エンジン（SpeechAnalyzer）を自動使用
- 録音中は画面下のインジケーターに認識中のテキストをライブ表示
- 複数モニターでは、作業中の画面にインジケーターを表示
- 録音中の`esc`キーで即キャンセル、終了10秒前からのカウントダウン表示
- 開始・終了のサウンド（設定でオフ可）
- メニューバーから話す言語と入力方法をすぐに切り替え
- 「改行」「新しい段落」「new line」などの音声レイアウトコマンド
- 録音時間表示と55秒の安全停止（残り10秒からカウントダウン警告）
...
- カスタム辞書（最大500語）: 「読み・誤認識例」を登録すると認識後に正しい表記へ置き換え。高精度エンジンでも有効
- 置換（最大200件）: 「仕事メール」などのひと言をメールアドレスやURL、定型文に展開
- 使用するマイクの選択とレベルテスト
- 起動方式「自動」: 短く押すとハンズフリー、長押しで押している間だけ録音
- AI整形へのカスタム指示（文体・書式の好みを自由記述）
- サイドバー式の設定画面（一般／ショートカット／音声認識／辞書／置換／AI整形／履歴とプライバシー／権限／サポート）
- 音声を保存しない設計
...
- macOS 13 Ventura以降（macOS 26以降では新しい高精度オンデバイス認識エンジンを自動使用）
